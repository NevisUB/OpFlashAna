...
namespace flashana {
  ...
  */
  ...
  public:
    ...
    bool Overlap(const flashana::TimeRange& range) const
    {
    ...
    void Insert(const flashana::TimeRange& range)
    {
      auto low = std::lower_bound(_time_range_v.begin(),
				  _time_range_v.end(),
				  range);
  ...
  private:
    ...
    std::vector<flashana::TimeRange> _time_range_v;
...
namespace std {
  template <>
  class less<flashana::TimeRange*>
  {
  public:
    bool operator()( const flashana::TimeRange* lhs, const flashana::TimeRange* rhs )
    { return (*lhs) < (*rhs); }
  };
}
