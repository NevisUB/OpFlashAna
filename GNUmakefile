...
#
SUBDIRS := OpMichel OpT0Finder #ADD_NEW_SUBDIR ... do not remove this comment from this line
...
all:
	@for i in $(SUBDIRS); do ( echo "" && echo "Compiling $$i..." && cd $(LARLITE_USERDEVDIR)/OpFlashAna/$$i && $(MAKE) ) || exit $$?; done
#####################################################################################
#
# CLEANs...
#
clean:
	@for i in $(SUBDIRS); do ( echo "" && echo "Cleaning $$i..." && cd $(LARLITE_USERDEVDIR)/OpFlashAna/$$i && $(MAKE) clean && rm -f $(LARLITE_LIBDIR)/$$i.* ) || exit $$?; done
...
doxygen:
	...
	@mkdir -p $(LARLITE_USERDEVDIR)/OpFlashAna/doc/dOxygenMyProject
	@doxygen $(LARLITE_USERDEVDIR)/OpFlashAna/doc/doxygenMyProject.script
